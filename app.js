...
function filterReviewList(filterType) {
  ...
  ["all", "wrong", "correct"].forEach(f => {
    ...
    if (btn) {
      if (f === filterType) {
        btn.className = "px-3 py-1.5 rounded-lg bg-white text-slate-800 shadow-xs font-bold";
      } else {
        btn.className = "px-3 py-1.5 rounded-lg text-slate-600 hover:text-slate-900";
      }
    }
  });
  renderFilteredReviewList();
...
function renderFilteredReviewList() {
  const reviewContainer = document.getElementById("full-review-container");
  ...
  reviewContainer.innerHTML = filtered.map(q => {
    const studentAns = state.examAnswers[q.number];
    let isCorrect = false;
    let userAnsDisplay = "-";
    ...
    const correctAnsDisplay = Array.isArray(q.correctAnswer) ? q.correctAnswer.sort().join(", ") : q.correctAnswer;

    return `
      <div class="p-5 sm:p-6 rounded-3xl border ${isCorrect ? 'border-emerald-200 bg-emerald-50/20' : 'border-rose-200 bg-rose-50/20'} mb-5 shadow-2xs">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center gap-2">
            <span class="font-extrabold text-xs px-2.5 py-1 rounded-lg ${isCorrect ? 'bg-emerald-200 text-emerald-950' : 'bg-rose-200 text-rose-950'}">
              No. ${q.number}
            </span>
        ...
        <p class="font-bold text-slate-900 text-sm sm:text-base mb-4 leading-snug">${q.question}</p>
